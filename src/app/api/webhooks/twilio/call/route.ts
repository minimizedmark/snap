...
import { sendUserSms, sendOwnerNotification, normalizePhoneNumber } from '@/lib/twilio';
...
async function processCallAsync(req: NextRequest) {
  try {
    ...
    // Send SMS to caller using THEIR Twilio account
    let smsStatus = 'pending';
    let smsMessageSid: string | null = null;

    try {
      const smsResult = await sendUserSms(
...
async function sendLowBalanceAlert(
  ...
  try {
    ...
    if (notifSettings?.notifySms && notifSettings.smsNumber) {
      await sendOwnerNotification(
        notifSettings.smsNumber,
        `⚠️ Snapback Wallet Alert: Your balance is $${balance.toFixed(2)}. Add funds at ${process.env.APP_URL}/wallet`
      );
    }
  } catch (error) {
